...
if [ "$CONNECT_TO_TESTNET" = true ]; then
    ...
    if [ -f "/root/rl-swarm-backup/userApiKey.json" ] && [ -f "/root/rl-swarm-backup/userData.json" ]; then
        ...
        echo_green ">> Identity files copied successfully!"
    else
        echo_blue ">> No backup files found at /root/rl-swarm-backup/, waiting for manual login..."
    fi
...
# 自动化：HuggingFace Hub 选择 N (你测试成功的方法)
HF_TOKEN=${HF_TOKEN:-""}
if [ -n "${HF_TOKEN}" ]; then # Check if HF_TOKEN is already set and use if so. Else give user a prompt to choose.
    HUGGINGFACE_ACCESS_TOKEN=${HF_TOKEN}
else
    echo_green ">> Automatically selecting N for Hugging Face Hub push"
    HUGGINGFACE_ACCESS_TOKEN="None"
fi

# 自动化：模型选择 Enter (你测试成功的方法)
